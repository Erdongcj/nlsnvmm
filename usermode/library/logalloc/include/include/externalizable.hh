...
#ifndef _LOGMANAGEMENT_COMMON_EXTERNALIZABLE_HPP_
#define _LOGMANAGEMENT_COMMON_EXTERNALIZABLE_HPP_
...
struct Externalizable {
   ...
   */
  virtual void assign(const Externalizable *other) = 0;
...
#define COMPLETE_EXTERNALIZABLE(clazz) \
  ...
  void assign(const Externalizable *other) CXX11_OVERRIDE {\
...
#define EXTERNALIZABLE(clazz) \
  ...
  const char* get_tag_name() const CXX11_OVERRIDE { return EX_EXPAND(clazz); }\
  void assign(const Externalizable *other) {\
    *this = *dynamic_cast< const clazz * >(other);\
  }\
  friend std::ostream& operator<<(std::ostream& o, const clazz & v) {\
    return o;\
