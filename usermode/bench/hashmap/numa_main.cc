...
int main(int argc, char **argv)
{
	...
	struct hashmap_tx* ht = hm_tx_create();
	stop = 0;
...
//	ssalloc_init();
	...
	gettimeofday(&start_time, NULL);
		...
		if (hm_tx_insert(ht, val, (uint64_t)(&val))) {
			last = val;
			i++;
		}
	}
...
//	printf("ave_insert_time : %lu\n", (double)insert_time / updates);
	printf("get_time : %lu\n", get_time);
//	printf("ave_get_time : %lu\n", (double)get_time / effreads);
